...
def get_respuestas(perfil, fecha_min=0, fecha_max=0):
    ...
    while len(datos_cc) < N_respuestas:
        ...
        try:
            ...
            print(datos)
            print('Descarga incompleta por límite de tasa. Termina el ', datetime.fromtimestamp(last_ts))
            break

    return datos_cc


def escribe_resultado(nombre_perfil, fecha_inicio, fecha_fin, contenido_datos_cc):
    Nom_arch = "Ccat_" + nombre_perfil +"_"+ fecha_inicio.replace("/", "-") +"_"+fecha_fin.replace("/", "-")+".txt"
    f = open(Nom_arch, "w", encoding="utf-8")
    for preg, resp in contenido_datos_cc.items():
        f.write("PREGUNTA: ")
        f.write(preg)
        f.write("\n\n")
        f.write("RESPUESTA: ")
...
escribe_resultado(perfil, fecha_min, fecha_max, datos_cc)
